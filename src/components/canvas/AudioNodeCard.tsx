...
import { ArrowUp, ChevronUp, Download, Eye, Loader2, Music2, Plus, Volume2, Wand2 } from "lucide-react";
            ...
            {selected && (
              ...
              >
                <Tooltip content="下载音频" position="top">
                  <button type="button" onClick={downloadAudio} className="flex h-9 w-9 items-center justify-center rounded-[12px] text-slate-400 transition-colors hover:bg-white/[0.06] hover:text-slate-100">
          ...
          <div className={`rounded-[10px] border border-slate-500/22 bg-[#0b0f17] px-4 py-5 shadow-[0_24px_58px_-34px_rgba(0,0,0,0.9)] ${selected ? "shadow-[0_0_0_1.5px_rgba(192,132,252,0.58),0_0_0_6px_rgba(139,92,246,0.14),0_0_38px_rgba(109,40,217,0.18)]" : ""}`}>
            <div className="mb-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1 truncate text-[11px] text-slate-400/70">{voiceId} · {AUDIO_MODEL}</div>
              </div>
            </div>
            <audio
              src={audioUrl}
        ...
        {(isHovered || selected) && !audioUrl && (
          ...
          >
            ...
            <textarea
              ...
              placeholder={upstreamPrompt ? `已由上游节点 (${upstreamPrompt.key}) 提供文本` : "输入要合成的语音文本"}
            ...
            <div className="mt-3 flex items-center gap-2 border-t border-cyan-100/8 pt-3">
              ...
              <div className="relative">
                ...
                >
                  <span>{VOICE_OPTIONS.find((item) => item.value === voiceId)?.label || "默认音色"}</span>
              ...
              <div className="relative">
                ...
                >
                  <span>{EMOTION_OPTIONS.find((item) => item.value === emotion)?.label || "自动情绪"}</span>
                  <ChevronUp className="h-3.5 w-3.5 rotate-180 text-slate-300/55" />
                </button>
                <AnimatePresence>
