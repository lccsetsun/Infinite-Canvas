export const mediaNodeFloatingToolbarClass =
  "absolute left-1/2 top-0 z-40 flex h-11 -translate-x-1/2 -translate-y-[calc(100%+var(--media-node-toolbar-gap,30px))] items-center gap-1 rounded-[15px] border border-slate-400/12 bg-[#111925]/82 px-2.5 shadow-[0_12px_28px_-20px_rgba(0,0,0,0.86),inset_0_1px_0_rgba(255,255,255,0.045)] backdrop-blur-xl";

export const mediaNodeFloatingToolbarRaisedClass =
  "absolute left-1/2 top-0 z-[70] flex h-11 -translate-x-1/2 -translate-y-[calc(100%+var(--media-node-toolbar-gap,30px))] items-center gap-1 rounded-[15px] border border-slate-400/12 bg-[#111925]/82 px-2.5 shadow-[0_12px_28px_-20px_rgba(0,0,0,0.86),inset_0_1px_0_rgba(255,255,255,0.045)] backdrop-blur-xl";
...
export function getMediaNodeFloatingToolbarGap(canvasZoom: number) {
  const normalizedZoom = Number.isFinite(canvasZoom) && canvasZoom > 0 ? canvasZoom : 1;
  if (normalizedZoom >= 0.8) return 30;
  return Math.round(30 + (0.8 - Math.max(0.35, normalizedZoom)) * 96);
}
